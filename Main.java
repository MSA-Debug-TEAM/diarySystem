...
class LoginManager {// the relation between "LoginManager"& "DatabaseHandler" -> aggregation
    ...
    public User login(String username, String password) throws Exception                     // 1- login()is to handle user login process
    {
    ...
    public boolean register(String username, String password) throws Exception {           //register() is to handle user registeration process
        if (dbHandler.getUserByUsername(username) != null)           //handle the case if username is already used by another user
            return false; 
        else{
    ...
    public void changePassword(String username, String newPassword) throws Exception {    // changePassword() is to handle user changing password process
...
class RecordManager { // the relation between RecordManager class& DatabaseHandler class is aggregation
    ...
    public List<Diary> viewRecord(int userId) throws Exception {             //      1-viewRecord()-> get user diaries from database for logged in user
    ...
    public void deleteRecord(int id) throws Exception {                              //      4- deleteRecord()-> delete existing thing
